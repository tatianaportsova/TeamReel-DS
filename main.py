...
#
# Returns string "True" if received.
    @app.route("/analyze_new_video", methods=['POST'])
...
#     @app.route('/')
#     def root():
#         title = """Welcome to the Team Reel DS Interview Analysis API!"""
#         api_welcome_string = "Endpoints:\n"
#         endpoint_1="""/get_user_performance: Takes in a JSON with a TeamReel user_id
#         (the id in our DB's 'users' table), and returns a JSON with analysis of
#         that user's interview performance to date."""
#         endpoint_2="""/get_prompt_top_responses: Takes in a JSON with a TeamReel
#         prompt_id (the id in our DB's prompts table), and returns a JSON with
#         that prompt's top 3 video responses."""
#         endpoint_3="""/get_video_analysis: Takes in a JSON with a TeamReel
#         video_id (the id in our DB's videos table), and returns a JSON with
#         the performance analysis for that video."""
#         endpoint_4="""/analyze_new_video: For DS use only. Triggered by AWS
#         when any new video is uploaded by a user (when any message is in our
#         TeamReel AWS SQS queue -- i.e., a newly uploaded video waiting to be
#         processed). Gets the new video and its DB info --> analyzes the video
#         using our ML functions (+ also gets any human feedback on that video in
#         the TeamReel DB) --> updated the 'video_feedback' table in our DB with
#         the analysis for that video. Returns string "True" if received."""
#
#         return render_template("home.html",
#                                title=title,
#                                text=api_welcome_string,
...
#     # (the id in our DB's 'users' table), and returns a JSON with analysis of
#     # that user's interview performance to date.
#     @app.route("/get_user_performance", methods=['POST'])
...
#     # prompt_id (the id in our DB's prompts table), and returns a JSON with
#     # that prompt's top 3 video responses.
#     @app.route('/get_prompt_top_responses', methods=['POST'])
